...
int s32cc_init_xpcs_phy(struct udevice *dev, const char *phy_name,
			    struct phy *phy)
{
	...
	ret = generic_phy_get_by_name(dev, phy_name, phy);
	if (ret) {
		dev_err(dev, "Failed to get %s PHY\n", phy_name);
		return ret;
	}

	phy_speed = s32_serdes_get_lane_speed(phy->dev, phy->id);
	if (phy_speed < 0) {
	...
	ret = generic_phy_configure(phy, NULL);
	if (ret) {
		dev_err(dev, "Failed to configure %s PHY\n", phy_name);
		return ret;
	}

	xpcs = s32cc_phy2xpcs(phy);
	if (!xpcs) {
